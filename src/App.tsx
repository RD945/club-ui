...
import { Search, Heart, User, Compass, Users, MapPin, Calendar, Star, Bell, Filter, TrendingUp as Trending, Award, Clock, Activity } from 'lucide-react';
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-4">
              {filteredClubs.map((club) => (
                <div
                  key={club.id}
                  className="group bg-white/80 backdrop-blur-lg rounded-xl p-5 shadow-lg border border-gray-200 hover:shadow-xl hover:border-gray-300 transition-all duration-300 transform hover:-translate-y-1 relative overflow-hidden"
                >
                  {/* Trending Badge */}
                  {club.trending && (
                      ...
                      <div className="flex items-center gap-4 mb-4">
                        ...
                        {club.newEvents > 0 && (
                          <div className="flex items-center gap-1 text-sm text-gray-500">
                            ...
                            <span>{club.newEvents} new events</span>
                          </div>
                        )}
                      </div>
            ...
            {myClubs.length > 0 ? (
              <div className="space-y-4">
                ...
                {myClubs.map((club) => (
                  <div
                    key={club.id}
                    className="bg-white/80 backdrop-blur-lg rounded-xl p-5 shadow-lg border border-gray-200 hover:shadow-xl transition-all duration-200"
                  >
                    <div className="flex gap-4">
                      <img
                        src={club.image}
            ...
            {favoriteClubs.length > 0 ? (
              <div className="grid gap-4">
                {favoriteClubs.map((club) => (
                  <div
                    key={club.id}
                    className="bg-white/80 backdrop-blur-lg rounded-xl p-5 shadow-lg border border-gray-200 hover:shadow-xl hover:border-gray-300 transition-all duration-300 relative"
                  >
                    <div className="flex gap-4">
                      <img
                        src={club.image}
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-gray-600 text-sm mb-3 h-10">{club.description}</p>
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3 text-sm text-gray-500">
                            <span className="flex items-center gap-1">
                              <Users className="w-4 h-4" />
                    ...
                    <button
                      onClick={() => toggleFavorite(club.id)}
                      className="absolute bottom-5 right-5 w-10 h-10 bg-red-500 text-white rounded-full flex items-center justify-center shadow-lg"
                    >
          ...
          <div className="space-y-6">
            <div className="bg-gradient-to-r from-gray-900 to-gray-800 rounded-xl p-6 text-white">
              ...
              <div className="grid grid-cols-3 gap-4 text-center">
                ...
                <div>
                  <div className="text-2xl font-bold text-white">{joinedClubs.reduce((sum, id) => sum + (clubs.find(c => c.id === id)?.newEvents || 0), 0)}</div>
